...
public class MaskingUtils {
    ...
    public static String maskName(String name) {
        ...
        if (hasCjkCharacter(trimmed)) {
            return maskCjkName(trimmed);
        } else {
            return maskWesternName(trimmed);
        }
    }

    private static boolean hasCjkCharacter(String text) {
    ...
    private static String maskCjkName(String name) {
        int len = name.length();
        if (len == 1) return "*";
        if (len == 2) return name.charAt(0) + "*";

        // 3자 이상: 첫 자 + 중간 전체 마스킹 + 마지막 자
        String middle = "*".repeat(len - 2);
        return name.charAt(0) + middle + name.charAt(len - 1);
    }

    private static String maskWesternName(String name) {
        // 공백 기준으로 이름 분리 (예: "John Michael Smith")
        String[] parts = name.split("\\s+");
        if (parts.length == 1) {
            // 단일 이름은 CJK 규칙과 동일하게 처리
            return maskCjkName(name);
        }

        // 첫 번째 파트(First name) 마스킹: 첫 글자 노출
        String maskedFirst = parts[0].charAt(0) + "*".repeat(parts[0].length() - 1);

        // 나머지 파트(Last name 등)는 유지
     ...
     *
     * 마스킹 규칙:
     *   - 전체 숫자 중 앞 국가코드·지역코드 제외한 구독자 번호(subscriber number) 중간부를 마스킹
     *   - 구독자 번호가 4자리 이상이면 앞 2자리·뒤 2자리 노출, 나머지 마스킹
     *   - 파싱 실패(알 수 없는 형식) 시 전체 길이의 중간 50% 마스킹 처리
     *
     * 예시:
     ...
     */
    public static String maskPhone(String phoneNumber, String defaultRegion) {
        if (phoneNumber == null || phoneNumber.isBlank()) return phoneNumber;

        try {
            PhoneNumber parsed = PHONE_UTIL.parse(phoneNumber, defaultRegion);
            String formatted = PHONE_UTIL.format(parsed, PhoneNumberUtil.PhoneNumberFormat.INTERNATIONAL);
            ...
            // 포맷된 문자열에서 구독자 번호 부분 교체
            return replaceSubscriberInFormatted(formatted, subscriberDigits, maskedSubscriber);
    ...
    private static String maskSubscriberNumber(String subscriber) {
        int len = subscriber.length();
        if (len <= 4) return "*".repeat(len);

        int visibleEnd = 2;
        int maskLen = len - visibleEnd - 2;
        return subscriber.substring(0, 2) + "*".repeat(maskLen) + subscriber.substring(len - 2);
    }

    private static String replaceSubscriberInFormatted(String formatted, String original, String masked) {
        // 포맷된 문자열 뒤에서부터 숫자를 masked로 치환
    ...
    private static String fallbackMaskPhone(String phone) {
        ...
        for (int i = 0; i < chars.length; i++) {
            if (Character.isDigit(chars[i])) {
                if (current >= start && current < start + maskCount) {
                    chars[i] = '*';
                }
                current++;
            }
